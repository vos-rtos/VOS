...
int test_enc(int argc, char **argv)
{
   ...
   if (argc != 4 && argc != 3)
   {
      kprintf( "Usage: encode [in file] [out file] [bits file]\nargc = %d", argc);
      exit(1);
   }
   inFile = argv[1];
   //fin = fopen(inFile, "rb");
   ...
   while (!f_eof(&fin))
   {
      ...
      res = f_read (&fin, in_short, sizeof(short)*FRAME_SIZE, &num);
      if (f_eof(&fin))
         break;
      ...
      if (argc==4) {
         ...
         res = f_write (&fbits, cbits, nbBits, &num);
      }
      ...
      res = f_write (&fout, &out_short[skip_group_delay], sizeof(short)*(FRAME_SIZE-skip_group_delay), &num);
      skip_group_delay = 0;
   }
   kprintf( "Total encoded size: %d bits\n", bitCount);
...
//   rewind(fout);
   ...
   f_rewind(&fout);
   u32 num0;
   res = f_read (&fin, in_short, sizeof(short)*FRAME_SIZE, &num0);
   res = f_read (&fout, out_short, sizeof(short)*FRAME_SIZE, &num);
//   while ( FRAME_SIZE == fread(in_short, sizeof(short), FRAME_SIZE, fin)
//           &&
//           FRAME_SIZE ==  fread(out_short, sizeof(short), FRAME_SIZE,fout) )
   while (res == 0 && FRAME_SIZE == num0 && FRAME_SIZE == num)
   {
	float s=0, e=0;
