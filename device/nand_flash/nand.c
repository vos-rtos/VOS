...
{
    *(volatile u8*)(NAND_ADDRESS|NAND_CMD)=NAND_FEATURE;//发送设置特性命令
    *(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=0X01;		//地址为0X01,设置mode
 	*(volatile u8*)NAND_ADDRESS=mode;					//P1参数,设置mode
	*(volatile u8*)NAND_ADDRESS=0;
	*(volatile u8*)NAND_ADDRESS=0;
	*(volatile u8*)NAND_ADDRESS=0;
...
u32 NAND_ReadID(void)
{
    u8 deviceid[5];
    u32 id;
    *(volatile u8*)(NAND_ADDRESS|NAND_CMD)=NAND_READID; //发送读取ID命令
    *(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=0X00;
	//ID一共有5个字节
    deviceid[0]=*(volatile u8*)NAND_ADDRESS;
    deviceid[1]=*(volatile u8*)NAND_ADDRESS;
    deviceid[2]=*(volatile u8*)NAND_ADDRESS;
    deviceid[3]=*(volatile u8*)NAND_ADDRESS;
    deviceid[4]=*(volatile u8*)NAND_ADDRESS;
...
{
    volatile u8 data=0;
    *(volatile u8*)(NAND_ADDRESS|NAND_CMD)=NAND_READSTA;//发送读状态命令
    NAND_Delay(NAND_TWHR_DELAY);	//等待tWHR,再读取状态寄存器
 	data=*(volatile u8*)NAND_ADDRESS;			//读取状态值
    return data;
}
//等待NAND准备好
...
u8 NAND_Reset(void)
{
    *(volatile u8*)(NAND_ADDRESS|NAND_CMD)=NAND_RESET;	//复位NAND
...
//NAND延时
//一个i++至少需要4ns
void NAND_Delay(volatile u32 i)
{
	while(i>0)i--;
}
...
u8 NAND_ReadPage(u32 PageNum,u16 ColNum,u8 *pBuffer,u16 NumByteToRead)
{
	...
	u8 errsta=0;
	u8 *p;
     *(volatile u8*)(NAND_ADDRESS|NAND_CMD)=NAND_AREA_A;
    //发送地址
    *(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)ColNum;
    *(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)(ColNum>>8);
    *(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)PageNum;
    *(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)(PageNum>>8);
    *(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)(PageNum>>16);
    *(volatile u8*)(NAND_ADDRESS|NAND_CMD)=NAND_AREA_TRUE1;
	...
	if(NumByteToRead%NAND_ECC_SECTOR_SIZE)//不是NAND_ECC_SECTOR_SIZE的整数倍，不进行ECC校验
	{
		...
		{
			*(volatile u8*)pBuffer++ = *(volatile u8*)NAND_ADDRESS;
		}
	}else
	{
		...
		*(volatile u8*)(NAND_ADDRESS|NAND_CMD)=0X05;				//随机读指令
		//发送地址
		*(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)i;
		*(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)(i>>8);
		*(volatile u8*)(NAND_ADDRESS|NAND_CMD)=0XE0;				//开始读数据
		...
		for(i=0;i<eccnum;i++)								//检验ECC
		{
			...
			{
				kprintf("err hd,rd:0x%x,0x%x\r\n",nand_dev.ecc_hdbuf[i+eccstart],nand_dev.ecc_rdbuf[i+eccstart]);
 				kprintf("eccnum,eccstart:%d,%d\r\n",eccnum,eccstart);
				kprintf("PageNum,ColNum:%d,%d\r\n",PageNum,ColNum);
...
u8 NAND_ReadPageComp(u32 PageNum,u16 ColNum,u32 CmpVal,u16 NumByteToRead,u16 *NumByteEqual)
{
    ...
    *(volatile u8*)(NAND_ADDRESS|NAND_CMD)=NAND_AREA_TRUE1;
    ...
    {
		if(*(volatile u32*)NAND_ADDRESS!=CmpVal)break;	//如果有任何一个值,与CmpVal不相等,则退出.
    }
...
u8 NAND_WritePage(u32 PageNum,u16 ColNum,u8 *pBuffer,u16 NumByteToWrite)
{
	...
	*(volatile u8*)(NAND_ADDRESS|NAND_CMD)=NAND_WRITE0;
    //发送地址
    *(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)ColNum;
    *(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)(ColNum>>8);
    *(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)PageNum;
    ...
    *(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)(PageNum>>16);
	...
	if(NumByteToWrite%NAND_ECC_SECTOR_SIZE)//不是NAND_ECC_SECTOR_SIZE的整数倍，不进行ECC校验
	{
		...
		{
			*(volatile u8*)NAND_ADDRESS=*(volatile u8*)pBuffer++;
		}
	}else
	{
		...
		NAND_Delay(NAND_TADL_DELAY);//等待tADL
		*(volatile u8*)(NAND_ADDRESS|NAND_CMD)=0X85;				//随机写指令
		//发送地址
		*(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)i;
		*(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)(i>>8);
		NAND_Delay(NAND_TADL_DELAY);//等待tADL
    ...
    *(volatile u8*)(NAND_ADDRESS|NAND_CMD)=NAND_WRITE_TURE1;
 	VOSDelayUs(NAND_TPROG_DELAY);	//等待tPROG
	if(NAND_WaitForReady()!=NSTA_READY)return NSTA_ERROR;//失败
    return 0;//成功
}
...
u8 NAND_WritePageConst(u32 PageNum,u16 ColNum,u32 cval,u16 NumByteToWrite)
{
    ...
    *(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)(PageNum>>16);
	...
	{
		*(volatile u32*)NAND_ADDRESS=cval;
	}
    *(volatile u8*)(NAND_ADDRESS|NAND_CMD)=NAND_WRITE_TURE1;
 	VOSDelayUs(NAND_TPROG_DELAY);	//等待tPROG
    if(NAND_WaitForReady()!=NSTA_READY)return NSTA_ERROR;//失败
...
u8 NAND_CopyPageWithoutWrite(u32 Source_PageNum,u32 Dest_PageNum)
{
    ...
    *(volatile u8*)(NAND_ADDRESS|NAND_CMD)=NAND_MOVEDATA_CMD0;	//发送命令0X00
    //发送源页地址
    *(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)0;
    *(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)0;
    *(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)Source_PageNum;
    *(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)(Source_PageNum>>8);
    *(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)(Source_PageNum>>16);
    *(volatile u8*)(NAND_ADDRESS|NAND_CMD)=NAND_MOVEDATA_CMD1;//发送命令0X35
    //下面两行代码是等待R/B引脚变为低电平，其实主要起延时作用的，等待NAND操作R/B引脚。因为我们是通过
    //将STM32的NWAIT引脚(NAND的R/B引脚)配置为普通IO，代码中通过读取NWAIT引脚的电平来判断NAND是否准备
    //就绪的。这个也就是模拟的方法，所以在速度很快的时候有可能NAND还没来得及操作R/B引脚来表示NAND的忙
    //闲状态，结果我们就读取了R/B引脚,这个时候肯定会出错的，事实上确实是会出错!大家也可以将下面两行
	...
	res=NAND_WaitRB(1);			//等待RB=1
    if(res)return NSTA_TIMEOUT;	//超时退出
    *(volatile u8*)(NAND_ADDRESS|NAND_CMD)=NAND_MOVEDATA_CMD2;  //发送命令0X85
    //发送目的页地址
    *(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)0;
    *(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)0;
    *(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)Dest_PageNum;
    *(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)(Dest_PageNum>>8);
    *(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)(Dest_PageNum>>16);
    *(volatile u8*)(NAND_ADDRESS|NAND_CMD)=NAND_MOVEDATA_CMD3;	//发送命令0X10
	VOSDelayUs(NAND_TPROG_DELAY);	//等待tPROG
...
u8 NAND_EraseBlock(u32 BlockNum)
{
    ...
    *(volatile u8*)(NAND_ADDRESS|NAND_CMD)=NAND_ERASE0;
    //发送块地址
    *(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)BlockNum;
    *(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)(BlockNum>>8);
    *(volatile u8*)(NAND_ADDRESS|NAND_ADDR)=(u8)(BlockNum>>16);
    *(volatile u8*)(NAND_ADDRESS|NAND_CMD)=NAND_ERASE1;
	VOSTaskDelay(NAND_TBERS_DELAY);		//等待擦除成功
	if(NAND_WaitForReady()!=NSTA_READY)return NSTA_ERROR;//失败
    return 0;	//成功
}
//全片擦除NAND FLASH
void NAND_EraseChip(void)
{
    ...
    for(i=0;i<nand_dev.block_totalnum;i++) //循环擦除所有的块
    {
        ...
        if(status)kprintf("Erase %d block fail!!，错误码为%d\r\n",i,status);//擦除失败
    }
}
...
u8 NAND_ECC_Correction(u8* data_buf,u32 eccrd,u32 ecccl)
{
	...
	{
		kprintf("2bit ecc error or more\r\n");
		return 1;
	}
	return 0;
}
