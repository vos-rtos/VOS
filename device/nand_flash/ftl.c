...
u8 FTL_Init(void)
{
	...
	if(nand_dev.lut)vfree(nand_dev.lut);
	nand_dev.lut=vmalloc((nand_dev.block_totalnum)*2); 	//给LUT表申请内存
	memset(nand_dev.lut,0,nand_dev.block_totalnum*2);			//全部清理
    if(!nand_dev.lut)return 1;				//内存申请失败
    temp=FTL_CreateLUT(1);
    if(temp)
    {
        kprintf("format nand flash...\r\n");
	...
	{
		kprintf("total block num:%d\r\n",nand_dev.block_totalnum);
		kprintf("good block num:%d\r\n",nand_dev.good_blocknum);
		kprintf("valid block num:%d\r\n",nand_dev.valid_blocknum);
    }
	return 0;
}
...
u8 FTL_CreateLUT(u8 mode)
{
    ...
    for(i=0;i<nand_dev.block_totalnum;i++)
    {
		...
		if(buf[0]==0XFF)//是好块
        {
			...
			nand_dev.good_blocknum++;
		}else kprintf("bad block index:%d\r\n",i);
    }
    //LUT表建立完成以后检查有效块个数
    for(i=0;i<nand_dev.block_totalnum;i++)
    {
...
u8 FTL_BlockCompare(u32 blockx,u32 cmpval)
{
	...
	for(i=0;i<3;i++)//允许3次机会
	{
		...
		if(res)kprintf("error erase block:%d\r\n",i);
		else
		{
...
u32 FTL_SearchBadBlock(void)
{
	...
	u32 i,j;
	u32 goodblock=0;
	blktbl=vmalloc(nand_dev.block_totalnum);//申请block坏块表内存,对应项:0,好块;1,坏块;
	NAND_EraseChip(); 						//全片擦除
