...
volatile SD_Error TransferError=HAL_OK;					//数据传输错误标志,DMA读写时使用
...
SD_Error SD_Init(void)
{
  ...
  NVIC_InitStructure.NVIC_IRQChannel = SDIO_IRQn;
	...
	NVIC_Init(&NVIC_InitStructure);	//根据指定的参数初始化VIC寄存器、
 	...
 	if(errorstatus==HAL_OK)errorstatus=SD_InitializeCards();			//初始化SD卡
  	if(errorstatus==HAL_OK)errorstatus=SD_GetCardInfo(&SDCardInfo);	//获取卡信息
 	if(errorstatus==HAL_OK)errorstatus=SD_SelectDeselect((u32)(SDCardInfo.RCA<<16));//选中SD卡
   	if(errorstatus==HAL_OK)errorstatus=SD_EnableWideBusOperation(SDIO_BusWide_4b);	//4位宽度,如果是MMC卡,则不能用4位模式
...
SD_Error SD_PowerON(void)
{
  ...
  errorstatus=CmdResp7Error();						//等待R7响应
	...
	if(errorstatus==HAL_OK)//SD2.0/SD 1.1,否则为MMC卡
	{																  
      ...
      SDIO_SendCommand(&SDIO_CmdInitStructure);			//发送CMD55,短响应	 
			...
			errorstatus=CmdResp1Error(SD_CMD_APP_CMD); 	 	//等待R1响应  
 			...
 			if(errorstatus!=HAL_OK)return errorstatus;   	//响应错误
  ...
  SDIO->DCTRL=0x0;	//数据控制寄存器清零(关DMA) 
	...
	}else if(DeviceMode==SD_DMA_MODE)
	{
		...
		SDIO->MASK|=(1<<1)|(1<<3)|(1<<8)|(1<<5)|(1<<9);	//配置需要的中断 
 		...
 		while(((DMA2->LISR&(1<<27))==RESET)&&(TransferEnd==0)&&(TransferError==HAL_OK)&&timeout)timeout--;//等待传输完成
