...
u8 lwip_comm_mem_malloc(void)
{
	...
	memp_memory = vmalloc(mempsize);	//为memp_memory申请内存
	ramheapsize = LWIP_MEM_ALIGN_SIZE(MEM_SIZE)+2*LWIP_MEM_ALIGN_SIZE(4*3)+MEM_ALIGNMENT;//得到ram heap大小
	//ram_heap=vmalloc(ramheapsize);	//为ram_heap申请内存
//	TCPIP_THREAD_TASK_STK = vmalloc(TCPIP_THREAD_STACKSIZE*4);//给内核任务申请堆栈
//	LWIP_DHCP_TASK_STK = vmalloc(SRAMIN,LWIP_DHCP_STK_SIZE*4);		//给dhcp任务堆栈申请内存空间
...
void lwip_dhcp_task(void *pdata)
{
	...
	{ 
		...
		VOSTaskDelay(1000);//delay_ms(500); //延时250ms
	}
...
void lwip_comm_destroy(void)
{
    ...
    //PCF8574_WriteBit(ETH_RESET_IO,1);       //硬件复位LAN8720A
	GPIO_AF_Set(GPIOB,11,7);				//PB11,AF7,恢复为串口3 RX功能.    
} 
